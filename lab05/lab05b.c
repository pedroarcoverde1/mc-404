...
// recebe um input e um intervalo como parâmetros
// cria uma máscara que envolve a quantidade de bits desejada end_bit - start_bit + 1
// adiciona tudo a um valor iniciado com 0 
...
int main()
{
    ...
    // leitura da entrada no formato string
    char input[40];
    read(STDIN_FD, input, 40);
    InstData *data;
    int valor = 0; // em 32 bits temos 0000 0000 0000 0000 0000 0000 0000 0000

    // enche a struct InstData com os dados do programa
    get_inst_data(input,data);

    // realizar o pack com os dados da struct
    // levar em consideração o tipo de instrução no InstType {R, I, S, B, U, J}
    switch (data->type)
    {
    case R:
        pack(data->opcode, 0, 6, &valor);// opcode 0-6
        pack(data->rd, 7, 11, &valor);// rd 7-11
        pack(data->funct3, 12, 14, &valor);// funct3 12-14
        pack(data->rs1, 15, 19, &valor);// rs1 15-19
        pack(data->rs2, 20, 24, &valor); // rs2 20-24
        pack(data->funct7, 25, 31, &valor); // funct7 25-31
        break;
    case I:
        pack(data->opcode, 0, 6, &valor);// opcode 0-6
        pack(data->rd, 7, 11, &valor);// rd 7-11
        pack(data->funct3, 12, 14, &valor);// funct3 12-14
        pack(data->rs1, 15, 19, &valor);// rs1 15-19
        pack(data->imm & ((1<<12)-1), 20, 31, &valor); // imm[11:0] 20-31
        break;
    case S:
        pack(data->opcode, 0, 6, &valor);// opcode 0-6
        pack(data->imm & ((1<<5)-1), 7, 11, &valor);// 
        pack(data->funct3, 12, 14, &valor);// funct3 12-14
        pack(data->rs1, 15, 19, &valor);// rs1 15-19
        pack(data->rs2, 20, 24, &valor); // rs2 20-24
        pack((data->imm>>5) & ((1<<7)-1), 25, 31, &valor); // imm[11:5]
        break;
    case B:
        pack(data->opcode, 0, 6, &valor);// opcode 0-6
        pack((data->imm>>11) & 1, 7, 7, &valor);// imm[11] 0000 0000 0000 0000 0000 1000 0000 0000
        pack((data->imm>> 1) & ((1<<4)-1), 8, 11, &valor);// imm[4:1]
        pack(data->funct3, 12, 14, &valor);// FUNCT3 12-14
        pack(data->rs1, 15, 19, &valor); // rs1 15-19
        pack(data->rs2, 20, 24, &valor); // rs2 20-24
        pack((data->imm>>5) & ((1<<6)-1), 25, 30, &valor); // imm[10:5]
        pack((data->imm>>12)&1, 31, 31, &valor); // imm[10:5]
        break;
    case U:
        pack(data->opcode, 0, 6, &valor);
        pack(data->rd, 7, 11, &valor);
        pack(data->imm & ((1<<20)- 1), 12, 31, &valor); // segundo o enunciado não preciso shiftar
        break;
    case J:
        pack(data->opcode, 0, 6, &valor);
        pack(data->rd, 7, 11, &valor);
        pack((data->imm>>12) &((1<<8)-1), 12, 19, &valor);
        pack((data->imm>>11)&1, 20, 20, &valor);
        pack((data->imm>>1) & ((1<<10)-1), 21, 30, &valor);
        pack((data->imm>>20)&1, 31, 31, &valor);
        break;
    default:
        break;
    }

    // conveter para hex e printar
    hex_code(valor);
